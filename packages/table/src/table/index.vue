<template>
  ...
  <div class="table-main">
    ...
    <div class="table-header">
      ...
      <div v-if="rightButton" class="header-button-ri">
        <slot name="rightButton"> </slot>
      </div>
    </div>
    <!-- 表格主体 -->
    <el-table
      :header-cell-style="{ backgroundColor: '#E2F3FF' }"
      ref="tableRef"
      v-bind="$attrs"
    ...
    >
      ...
      <template #empty>
        <div class="table-empty">
          <slot name="empty">
            <img src="@/images/notData.png" width="121" alt="notData" />
            <div>暂无数据</div>
          </slot>
        </div>
      </template>
...
import { useTable } from "@clare-ui/table";
import { useSelection } from "@clare-ui/table";
import { BreakPoint } from "../types";
import { ColumnProps, TypeProps } from "../types";
import { Refresh, Operation, Search } from "@element-plus/icons-vue";
import { handleProp } from "@clare-ui/core";
import SearchForm from "../SearchForm/index.vue";
import Pagination from "../table/components/Pagination.vue";
import ColSetting from "../table//components/ColSetting.vue";
import TableColumn from "../table/components/TableColumn.vue";
...
export interface ProTableProps {
  ...
  rowClick?: (data: any) => any; // 点击行的回调函数，可以对数据进行处理 ==> 非必传
  ...
  gridSuffix?: boolean; // 表格是否需要网格线 ==> 非必传（默认为true）
  maxHeight?: number;
}
...
const setEnumMap = async ({ prop, enum: enumValue }: ColumnProps) => {
  ...
  } else if (data && !Array.isArray(data) && Array.isArray(data.rows)) {
    enumMap.value.set(prop!, data.rows);
  } else {
...
watch(searchColumns, (newColumns) => {
  newColumns.forEach((column, index) => {
    const key = column.search?.key ?? handleProp(column.prop!);
    if (column.search?.el === "date-picker") {
      const defaultValue = column.search?.defaultValue ?? null;
      searchInitParam.value[key] = defaultValue;
      searchParam.value[key] = defaultValue;
      watch(
        () => searchParam.value[key],
        (newValue) => {
          searchInitParam.value[key] = newValue || null; // 如果为空则取 null
        }
      );
    }
